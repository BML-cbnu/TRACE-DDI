...
# Base directories
nodes_base_path = f'/home/bml_ddi/TRACE-DDI/data/{nodes_type}/rw_mean/steps_{steps}/prob_{prob}/nodes'
edges_base_path = f'/home/bml_ddi/TRACE-DDI/data/{nodes_type}/rw_mean/steps_{steps}/prob_{prob}/edges'

# Output directories — created once
output_edges_dir = f'/home/bml_ddi/TRACE-DDI/data/{nodes_type}/rw_mean/steps_{steps}/prob_{prob}/hop{num_hop}/edges'
output_nodes_dir = f'/home/bml_ddi/TRACE-DDI/data/{nodes_type}/rw_mean/steps_{steps}/prob_{prob}/hop{num_hop}/nodes'
...
def load_nodes_edges(nodes_base_path, edges_base_path, compound_num):
    ...
    nodes = pd.read_csv(nodes_file_path, sep='\t', names=['node_num', 'node_name', 'node_type', 'spread_value', 'visted_count'])
...
def make_meta_edges(edges, meta_paths):
    ...
    for path in meta_paths:
        for u, v in zip(path[:-1], path[1:]):
            rows = edges[(edges['head'] == u) & (edges['tail'] == v)][['head','relation','tail']]
    ...
    meta_edges = meta_edges.drop_duplicates(subset=['head','relation','tail'], keep='first')
    return meta_edges
...
if __name__ == '__main__':
    ...
    for compound_num in tqdm(range(0, 1705 + 1), desc="Processing compounds"):
        ...
        pathway_list = nodes[nodes['node_name'].str.startswith('Pathway')]['node_num'].tolist() # pathway_list
        ...
        mata_paths = meta_paths(kg_nx, source=compound_num, target_list=pathway_list, num_hop=num_hop)
        ...
        meta_edges.to_csv(os.path.join(output_edges_dir, f'compound{compound_num}_edges.tsv'), sep='\t', index=False, header=False)
